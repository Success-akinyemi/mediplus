...
export async function paystackPay(req, res){
    ...
    try {
        ...
        res.json({ authorizationUrl: transaction.data.authorization_url });
      } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Unable to initialize transaction' });
      }
